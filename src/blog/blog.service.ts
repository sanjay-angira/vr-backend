...
export class BlogService {
  ...
  async create(createBlogDto: CreateBlogDto): Promise<any> {
    ...
    return successResponse(response, 'Blog created successfully', 201);
  }
  ...
  async findAll(paginationDto: PaginationDto): Promise<any> {
    ...
    const [rows, count] = await this.blogRepository.findAndCount({
      ...
      order: { [column]: order.toUpperCase() === 'ASC' ? 'ASC' : 'DESC' },
      skip,
      take,
  ...
  async update(id: number, updateBlogDto: UpdateBlogDto): Promise<any> {
    // Query entity directly — NOT via this.findOne() which returns a response wrapper
    const blog = await this.blogRepository.findOne({
      where: { id },
      relations: ['category', 'tags', 'seo'],
    });
    ...
    // Update SEO — merge into existing record to avoid duplicate key
    if (seo) {
      if (blog.seo) {
    ...
    const response = await this.blogRepository.save(blog);
    return successResponse(response, 'Blog updated successfully', 200);
  }
